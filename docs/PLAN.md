...
### Phase 4: 지역별 공개 데이터 활용
- 유럽(예: 스페인 DBRA24, CityTrek-14K), 동남아(Grab Safe Driver), 중동(UAE 센서 로그) 등 권역별 데이터셋을 선별해 동일한 파이프라인으로 분석
- 지역별 이벤트 가중치·환경 계수를 비교해 공통 패턴과 차이점을 기록하고 문서화
- 데이터 정제 시 지역별 인코딩(EUC-KR 등)과 활용 라이선스(공공데이터포털, Kaggle) 조건을 확인
...
| 2-4주 | Phase 2 | 실제 데이터 분석, 3개 이벤트 모델 개발 | 최적 가중치, 등급 기준, 성능 리포트 | 🚀 **시작 준비** |
